...
class Query_Resolver:
    ...
    def qna_model(self, query, step_no):
        ...
        context = self.recipe_details[self.cuisine][self.recipe]["Directions"][str(step_no - 1)]
        ...
        if ( (step >= 1) and (step <= total_no_dirs)):
            ...
            if (re.match(regex_next, question) ) :
                ...
                if (step <= total_no_dirs):
                    return (self.recipe_details[self.cuisine][self.recipe]["Directions"][str(step)])
                else:
            ...
            if (re.match(regex_ingredients, question)):
                ...
                self.process_ingredients(ingredients_nouns)
                '''
                text = word_tokenize(context)
                pos_tagged = pos_tag(text)
                nouns_and_pos = list(filter(lambda x: re.match("N.*", x[1]), pos_tagged))
                ingredients_nouns = list(set([n[0] for n in nouns_and_pos if n[0].lower() in All_Food]))
                print(ingredients_nouns)
                
                print(ingredients_bigram)
                '''
                return("")
            ...
            if (re.match(regex_time, question) and re.match(regex_total, question)):
                ...
                time_units = [' '.join(b) for b in list(filter(lambda x: re.match("\d.*", x[0]) or re.match(".*(minutes|mins|min|minute).*", x[1]), bigram))]
                print(time_units)
...
def main():
    ...
    qr_h.question_interpreter(parsed_query, step_no)
